        ...
        if FrameState == 0:  # When the state is not determined, enter the following judgment
            ...
            if Bytenum < 10:            # Read 8 data
                ...
                if data == (CheckSum & 0xff):  # verify check bit
                    ...
                    print("acc check failed.")
                    hex_list = [hex(num) for num in ACCData]
                    print(hex_list)
            ...
            if Bytenum < 10:
                ...
                if data == (CheckSum & 0xff):
                    ...
                    print("gyro check failed.")
                    hex_list = [hex(num) for num in GYROData]
                    print(hex_list)
                    print(f"{CheckSum:02x}")
                    print(f"{data:02x}")
                    print(Bytenum)
            ...
            if Bytenum < 10:
                ...
                if data == (CheckSum & 0xff):
                    ...
                    if True:
                        ...
                        minute = now.minute
                        second = now.second
                        millisecond = now.microsecond // 1000  # 微秒转换为毫秒
                        print(f"{hour:02d}:{minute:02d}:{second:02d}.{millisecond:03d}")

                    # result = acc+gyro+Angle
                    # print("acc:%10.3f %10.3f %10.3f \ngyro:%10.3f %10.3f %10.3f \nAngle:%10.3f %10.3f %10.3f" % result)
                    # return result 这行会导致程序出现bug，不能返回，否则数据处理中断
                else:
                    print("angle check failed.")
                    hex_list = [hex(num) for num in AngleData]
                    print(hex_list)
                    print(f"{CheckSum:02x}")
                    print(f"{data:02x}")
                    print(Bytenum)
